...
def extract_data(archive, dir_name: str) -> Data:
    ...
    """
    dir_names = os.path.join(archive, dir_name)
    with open(f"{dir_names}/caption.txt", "r", encoding='utf-8') as f:
        captions = f.readlines()
    data = []
    # supported = [".jpg", ".JPG", ".png", ".PNG", ".bmp"]  # 支持的文件后缀类型
    # images = [os.path.join(archive, dir_name, i) for i in os.listdir(dir_names)
    #           if os.path.splitext(i)[-1] in supported]
    # for line,img_path in captions,images:
    for line in captions:
        tmp = line.strip().split()
        img_name = tmp[0]
        formula = tmp[1:]
        # with open(f"{dir_name}/{img_name}.bmp", "r", encoding='utf-8') as f:
        #     # move image to memory immediately, avoid lazy loading, which will lead to None pointer error in loading
        img = Image.open(os.path.join(dir_names, (img_name+'.bmp'))).copy()
...
class CROHMEDatamodule(pl.LightningDataModule):
    def __init__(
        self,
        zipfile_path: str = f"{os.path.dirname(os.path.realpath(__file__))}/../../data_+",
        test_year: str = "2019",
        batch_size: int = 8,
        num_workers: int = 5,
    ) -> None:
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        # archives = os.path.join(self.zipfile_path , archive)
        if stage == "fit" or stage is None:
            self.train_dataset = build_dataset(self.zipfile_path, "train", self.batch_size)
            self.val_dataset = build_dataset(self.zipfile_path, self.test_year, 1)
        if stage == "test" or stage is None:
            self.test_dataset = build_dataset(self.zipfile_path, self.test_year, 1)
