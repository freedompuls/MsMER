...
from msmer.datamodule import Batch, vocab, vocab_size
from msmer.lit_msmer import LitMsMER
from msmer.utils import ExpRateRecorder, Hypothesis, to_tgt_output
...
class LitEnsemble(pl.LightningModule):
    def __init__(self, paths: List[str]):
        ...
        for p in paths:
            model = LitMsMER.load_from_checkpoint(checkpoint_path=p)
            model = model.eval()
    ...
    def beam_search(self, img, mask):
        src_mask_list = [m.msmer.encoder(img, mask) for m in self.models]
    ...
    def ensemble_beam_search(
        ...
        while len(completed_hypotheses) < beam_size and t < max_len:
            ...
            for i, m in enumerate(self.models):
                ...
                decode_outputs = m.msmer.decoder(exp_src, exp_src_mask, hypotheses)[
                    :, t, :
                ]
    ...
    def ensemble_cross_rate_score(
        ...
        for i, m in enumerate(self.models):
            ...
            output_hat = m.msmer.decoder(exp_src, exp_src_mask, tgt)
